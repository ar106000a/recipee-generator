...
          message: "Could not parse recipe from AI. No valid JSON found.",
          rawResponse: text,
        });
      }
...
      return res.status(500).json({
        message: "Could not parse recipe from AI. Please try again.",
        rawResponse: text,
      });
    }

    // Check for the 'error' key first
    if (recipeData.error) {
      return res.status(400).json({ message: recipeData.error });
    }

    // Validate the successful recipe format
    if (
      !recipeData.title ||
